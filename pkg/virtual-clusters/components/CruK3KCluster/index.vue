...
export default {
  ...
  computed: {
    ...
    hasPolicy() {
      const { namespace } = this.k3kCluster?.metadata || {};

      if (!namespace) {
        return;
      }

      const nsStoreObject = this.$store.getters['cluster/byId']({ type: NAMESPACE, id: namespace });

      return !!nsStoreObject?.metadata?.annotations?.[K3K.POLICY];
    },
  },
